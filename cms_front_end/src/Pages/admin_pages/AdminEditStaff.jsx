...
const AdminEditStaff = () => {
    const [inputs, setInputs] = useState({
      ...
      useEffect(() => {
        // Fetch user details only if user is not already in state or user ID changes
        if (!user || user.id !== parseInt(userId)) {
          dispatch(getUserDetails(userId));
        } else {
          // Set inputs if user is already fetched
          setInputs({
            roles: user.roles || "",
        ...
        <center>
          <nav id="sidebar">
            ...
            <ul className="list-unstyled components">
              <li>
                <Link to="#">
                  <FaUsers className="mr-2" /> Update Staff
                </Link>
              </li>
              <li>
